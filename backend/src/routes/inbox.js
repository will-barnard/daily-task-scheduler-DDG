...
router.post('/:id/approve', authenticate, (req, res) => {
  ...
  const result = db.prepare(
    'INSERT INTO tasks (title, description, send_date, product_url, created_by, created_by_name) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(title, description || '', send_date, item.product_url || null, req.user.id, req.user.name);
